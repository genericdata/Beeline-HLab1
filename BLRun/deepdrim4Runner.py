...
def generateInputs(RunnerObj):
    '''
    Function to generate desired inputs for DeepDRIM2.
    If the folder/files under RunnerObj.datadir exist, 
    this function will not do anything.

    :param RunnerObj: An instance of the :class:`BLRun`
    '''
    if not RunnerObj.inputDir.joinpath("DEEPDRIM2").exists():
        print("Input folder for DEEPDRIM2 does not exist, creating input folder...")
        RunnerObj.inputDir.joinpath("DEEPDRIM2").mkdir(exist_ok = False)
        
    if not RunnerObj.inputDir.joinpath("DEEPDRIM2/predict_pairsDEEPDRIM2.txtTF_divide_pos.txt").exists():
        ...
        newExpressionData.to_csv(RunnerObj.inputDir.joinpath("DEEPDRIM2/ExpressionData.csv"),
                             sep = ',', header  = True, index = True)

        geneNameMap = pd.concat([ExpressionData.index.to_series().str.lower(),ExpressionData.index.to_series()],axis=1)
        geneNameMap.to_csv(RunnerObj.inputDir.joinpath("DEEPDRIM2/GeneName_map.txt"),
        ...
        trueEdgesDF.to_csv(RunnerObj.inputDir.joinpath("DEEPDRIM2/PositivePairs.txt"),
                                 sep=',',header=False,index=False)
        
        
        # relative to current path; current path is mounted as /ext3/data
        #inputDir = str(RunnerObj.inputDir).split(str(Path.cwd()))[1] + "/DEEPDRIM2"
        inputDir = RunnerObj.inputDir.relative_to(Path.cwd()).joinpath('DEEPDRIM2')

        # Create training pairs, training pairs TF divide, gene name map files
        # the function outputs to cwd, so enter into the DEEPDRIM2 directory to have the output files there
        cmdToRun = ' '.join([
        ...
        tfDividePos = np.genfromtxt(RunnerObj.inputDir.joinpath('DEEPDRIM2/training_pairsDEEPDRIM2.txtTF_divide_pos.txt'),
                                    delimiter=',',dtype='int')
        tfGroups = np.repeat(np.arange(len(tfDividePos)-1),np.diff(tfDividePos))
        group_kfold = GroupKFold(n_splits=3)
        with open(RunnerObj.inputDir.joinpath('DEEPDRIM2/training_pairsDEEPDRIM2.txtCV_fold_divide.txt'),'w') as ofh:
        ...
        trainingPairs = pd.read_csv(RunnerObj.inputDir.joinpath('DEEPDRIM2/training_pairsDEEPDRIM2.txt'),
                                    sep='\t',header=None,index_col=None)
        trainingEdgesFrom = trainingPairs.iloc[tfDividePos[:-1],0]
        allGenes = ExpressionData.index.to_series().str.lower()
        predictPairs = pd.DataFrame(np.array(np.meshgrid(trainingEdgesFrom, allGenes, indexing='xy')).T.reshape(-1,2),
                                      columns=['Gene1','Gene2'])
        predictPairs['value'] = 3
        predictPairs.to_csv(RunnerObj.inputDir.joinpath("DEEPDRIM2/predict_pairsDEEPDRIM2.txt"),
                                      sep='\t',header=False,index=False)

        predictPairs['rnum'] = predictPairs.index
        predictDividePos = [min(predictPairs.loc[predictPairs.Gene1==tf,'rnum']) for tf in trainingEdgesFrom]
        predictDividePos.append(predictPairs.shape[0])
        np.savetxt(RunnerObj.inputDir.joinpath("DEEPDRIM2/predict_pairsDEEPDRIM2.txtTF_divide_pos.txt"),predictDividePos,fmt='%d')
        
def run(RunnerObj):
    '''
    Function to run DEEPDRIM2 algorithm

    :param RunnerObj: An instance of the :class:`BLRun`
    '''
    # current path will be mounted as /ext3/data
    # '/ext3/data/inputs/L0/hESC/DEEPDRIM2'
    inputDir = Path('/ext3/data').joinpath(RunnerObj.inputDir.relative_to(Path.cwd()),'DEEPDRIM2')

    # outputs/L0/hESC/DEEPDRIM2
    # make output dirs if they do not exist:
    outDir = "outputs/"+str(RunnerObj.inputDir).split("inputs/")[1]+"/DEEPDRIM2/"
    os.makedirs(outDir, exist_ok = True)
    
    with open(RunnerObj.inputDir.joinpath('DEEPDRIM2/training_pairsDEEPDRIM2.txtTF_divide_pos.txt'), 'r') as fp:
        numBatches = len(fp.readlines()) -1

    loadFromH5 = False
    TFOrderRandom = False
    if Path(outDir).joinpath('representation_train').exists():
        print("Representation folder for DEEPDRIM2 training exists, removing representation training folder...")
    ...
    cmdToRun = ' '.join([
        ...
        '-pairs_for_predict_file ',str(inputDir.joinpath('training_pairsDEEPDRIM2.txt')),
        '-geneName_map_file ',str(inputDir.joinpath('DEEPDRIM2_geneName_map.txt')),
        '-flag_load_from_h5 '+str(loadFromH5),
        '-flag_load_split_batch_pos True',
        '-TF_divide_pos_file '+str(inputDir.joinpath('training_pairsDEEPDRIM2.txtTF_divide_pos.txt')),
    ...
    if Path(outDir).joinpath('representation_predict').exists():
        print("Representation folder for DEEPDRIM2 predict exists, removing representation predict folder...")
    ...
    cmdToRun = ' '.join([
        ...
        '-expr_file ',str(inputDir.joinpath('ExpressionData.csv')),
        '-pairs_for_predict_file ',str(inputDir.joinpath('predict_pairsDEEPDRIM2.txt')),
        '-geneName_map_file ',str(inputDir.joinpath('DEEPDRIM2_geneName_map.txt')),
        '-flag_load_from_h5 '+str(loadFromH5),
        '-flag_load_split_batch_pos True',
        '-TF_divide_pos_file '+str(inputDir.joinpath('predict_pairsDEEPDRIM2.txtTF_divide_pos.txt')),
        '-TF_num '+str(numBatches), '-TF_order_random '+str(TFOrderRandom),
        '\"'])
    print(cmdToRun)
    os.system(cmdToRun)
    ...
    cmdToRun = ' '.join([
        ...
        '-output_dir .',
        '-cross_validation_fold_divide_file '+str(inputDir.joinpath('training_pairsDEEPDRIM2.txtCV_fold_divide.txt')),
        '\"'])
    print(cmdToRun)
    os.system(cmdToRun)

    with open(RunnerObj.inputDir.joinpath('DEEPDRIM2/training_pairsDEEPDRIM2.txtCV_fold_divide.txt'),'r') as ifh:
        cross_fold = []
        for line in ifh:
            line = line.strip()
...
def parseOutput(RunnerObj):
    '''
    Function to parse outputs from DEEPDRIM2.
    ...
    geneNameMap = pd.read_csv(RunnerObj.inputDir.joinpath('DEEPDRIM2/GeneName_map.txt'),
